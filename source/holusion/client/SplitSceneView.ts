...
import RenderQuadView, { ILayoutChange } from "./RenderQuadView";
 ...
 * between viewports.
 */
@customElement("sv-split-scene-view")
export default class SceneView extends SystemView
{
    ...
    protected splitMode:boolean = false;

    constructor(system?: System, split?:boolean)
    {
        super(system);

        if(split!=undefined)this.splitMode=split;
    ...
    {
        this.classList.add("sv-split-scene-view");
        ...
        if(this.splitMode)
        {
            let effectLayer=this.appendElement("div", {
                position: "absolute",
                top: "0", bottom: "0", left: "0", right: "0",
                overflow: "hidden"
            });
            effectLayer.classList.add("et-effect-layer")
        }
    ...
    {
        if(this.canvas) this.canvas.dataset.offset=""+offset;
    }
