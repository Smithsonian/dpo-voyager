...
export default class NPresentationScene extends NScene
{
    static readonly type: string = "NPresentationScene";

    url: string;
    assetPath: string;

    get scene() {
        return this.components.safeGet(CVoyagerScene);
    }
    get loadingManager() {
    ...
    protected nodeToData(node: NTransform, data: Partial<IPresentation>, refIndex: number): number
    {
        ...
        else if (node instanceof NReference) {
            data.references = data.references || [];
            nodeData.reference = data.references.length;
